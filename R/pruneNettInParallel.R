...
#'
#' @examples
#'\dontrun{# Example with data from the Breast Cancer Recovery Project
#'library(quint)
#'data(bcrp)
#'set.seed(10)
#'head(bcrp)
...
#'
#'# Prune the tree
#'fit <- pruneNettInParallel(tree=fitnett, B=2,pp=1)
#'
...
#' @exportClass nett
#' @export
pruneNettInParallel <- function(tree,pp=1,B=20,...){
  object<-tree
  ...
  while(emptyleaf==TRUE){
    ...
    result.paral<-foreach(b = 1:B,
                          .combine=list, .multicombine = TRUE,
                          .packages='ojUtils', .export=c('nett','bos1','makeGchmat','cpmat','ctc','computeC','firsttwosplits','ctmat','computeD','csigmap','bovar23','bos23','bovar','bos','leafnum','cSequence',
                                                         'cSubtree','cChild','child','predictCV.nett','predictInnerNett'))  %dopar%{
                                                           ...
                                                           list(predictedY=predictedY[[b]],Cboot=Cboot[[b]])
                                                         }

    predictedY<-list()
    Cboot<-list()
    for(b in 1:B){
      predictedY[[b]]<-result.paral[[b]]$predictedY
      Cboot[[b]]<-result.paral[[b]]$Cboot
    }
  ...
  if(bestbranch==1){stop("There is no clear qualitative interaction present in the data.","\n")}
  if(bestbranch>1){
